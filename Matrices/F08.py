...
class Functional(Layer):
    ...
    def fprop(self, inputs):
        ...
        for i in xrange(inputs.shape[0]):
            self.X[i] = self.INP[i] * self.W0T + self.W1T
            self.DX[i] = self.U.T * (self.X[i] > 0)
            a[i] = numpy.sum(self.U.T * self.relu(self.X[i]), axis = 1) + self.b
