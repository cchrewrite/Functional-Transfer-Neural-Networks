...
class Functional(Layer):
    ...
    def fprop(self, inputs):
        ...
        for i in xrange(inputs.shape[0]):
            self.X[i] = self.logistic(self.INP[i] * self.W1T + self.W2T)
            self.DX[i] = self.X[i] * (1 - self.X[i])
            a[i] = numpy.sum(self.W0T * self.X[i], axis = 1) + self.b
        
        """

        self.X = self.logistic(self.INP[:,None] * self.W1T + self.W2T)
        self.DX =  self.X * (1.0 - self.X)
