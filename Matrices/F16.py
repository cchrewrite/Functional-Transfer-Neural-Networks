...
class Functional(Layer):
    ...
    def fprop(self, inputs):
        ...
        for i in xrange(inputs.shape[0]):
            self.X[i] = numpy.tanh(self.INP[i] * self.W1T + self.W2T)
            self.DX[i] = 1 - self.X[i] * self.X[i]
            a[i] = numpy.sum(self.W0T * self.X[i], axis = 1) + self.b
        """


        self.X = numpy.tanh(self.INP[:,None] * self.W1T + self.W2T)
